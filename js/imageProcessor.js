...
function applyImageProcessing(canvas, settings) {
    ...
    switch (settings.ditherMethod) {
        ...
        case 'floydSteinberg':
            ...
            break;
        case 'stucki':
            applyStucki(imageData, settings.threshold);
            break;
        case 'atkinson':
...
/**
 * Aplica um filtro de nitidez (Laplacian Sharpen) e o algoritmo de Stucki
 */
export function applyStucki(imageData, threshold) {
    const data = imageData.data;
    const width = imageData.width;
    const height = imageData.height;

    // --- PASSO 1: CAMADA DE NITIDEZ (Sharpening) ---
    // Usamos uma convolução simples para destacar bordas antes do dithering
    const originalData = new Uint8ClampedArray(data);
    for (let y = 1; y < height - 1; y++) {
        for (let x = 1; x < width - 1; x++) {
            const idx = (y * width + x) * 4;
            // Matriz de Nitidez: [0, -1, 0], [-1, 5, -1], [0, -1, 0]
            const val = 5 * originalData[idx] - (
    ...
    // --- PASSO 2: DITHERING DE STUCKI ---
    // Matriz de pesos do Stucki:
    //       * 8   4
    //   2   4   8   4   2 (dividido por 42)
    //   1   2   4   2   1
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            // Distribuir o erro (Matriz Stucki)
            distributeError(data, width, height, x, y, error);
        }
    }
}

// Função auxiliar para distribuir o erro do Stucki
